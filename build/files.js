...
module.exports = exports = class Files {
  ...
  constructor (paths, base = '') {
    ...
    this._getPoster = memoize(() =>
      find(assets, { name: 'poster', dir: this.base }),
    );

    this._getFirst = memoize(() =>
      find(assets, (f) => f.dir === this.base && !!f.name.match(/^0*1(?:-0*1)?$/)),
    );
